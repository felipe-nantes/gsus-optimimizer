...
## DEC-111 — Diagnóstico de execução em linguagem simples (DIAG-001): o auditor precisa saber se a culpa foi do GSUS, não do programa

**Pedido do usuário (2026-09-03):** o auditor que opera o sistema não tem conhecimento técnico (PROJECT_SPEC.md seção 2) -- se uma execução noturna falha por instabilidade do GSUS ou porque o computador desligou sozinho, ele não pode concluir "o sistema não funciona" nem abrir chamado de suporte por algo que não é defeito do programa. Pedido explícito: o sistema precisa "se auto vistoriar" e deixar claro, sem precisar abrir log técnico nenhum, o que aconteceu e se precisa de auditoria manual ou suporte de verdade.

**Método:** antes de implementar, uma auditoria exaustiva (4 agentes em paralelo, cada um relendo o código-fonte completo de um subsistema -- orchestrator.py, login.py/client.py/adapter.py, census.py/records.py, scheduling.py/main.py/update_flow.py) catalogou TODO caminho de falha/curto-circuito existente. Dois achados centrais motivaram o desenho final:

1. **Várias falhas hoje não deixam NENHUM rastro persistente** -- só no arquivo de log técnico. Confirmado: falha de login, censo falhando por completo, falha de bookkeeping (`repo.next_pending`/`finish_run`) todas resultam em `run_id` nunca criado (login/censo) ou numa run que fica presa em RUNNING pra sempre sem nunca virar FAILED (`finish_run` falhando por contenção com a Fase 2, DEC-109) -- justamente os cenários mais graves pro auditor não saber o que aconteceu.
2. **O nome da classe de exceção sozinho não basta pra classificar "foi o GSUS"** -- o catálogo encontrou várias exceções `GSUS*` que cobrem tanto instabilidade real do GSUS (ex.: `GSUSRecordError` por marcador não confirmado a tempo, DEC-102) quanto casos que merecem investigação (ex.: `GSUSRecordError` por "número de prontuário não aceito pelo campo de busca", DEC-036/037/038 -- possível bug nosso). Também confirmado, verificando a classe de verdade: `type(exc).__name__` do Playwright pra `TimeoutError`/`Error` é literalmente "TimeoutError"/"Error" -- nunca aparece como "PlaywrightTimeoutError" no texto persistido (suposição inicial errada, corrigida ao testar contra a classe real antes de finalizar).

**Decisão:** nova tabela aditiva `run_diagnostics` (`run_id` NULL quando a execução nem chegou a criar uma run -- exatamente o caso antes invisível), e um módulo novo e pequeno, `app/analysis/run_diagnosis.py`, com funções PURAS (sem acesso a banco) de classificação:
- `classify_top_level_exception(exc)`: usa a exceção DE VERDADE (`isinstance`, não texto) pra classificar falhas que impediram a Fase 1 de terminar -- `GSUSLoginError`/`GSUSCensusError`/qualquer `playwright.sync_api.Error` (cobre `TimeoutError` via herança) viram FALHA_GSUS; `RuntimeError` de credencial ausente e `sqlite3.Error`/`OSError` viram FALHA_INESPERADA com resumo específico; qualquer outra coisa (default seguro) também vira FALHA_INESPERADA.
- `classify_completed_run(...)`: pra runs que terminaram, usa uma lista curada de padrões de MENSAGEM (não classe) construída item a item a partir do catálogo da auditoria -- só entra na lista "causa conhecida" o padrão que algum DEC já registrado atribui especificamente à instabilidade do GSUS. Qualquer erro de paciente fora dessa lista (incluindo erro bruto do Playwright escapando de um ponto ainda sem retry, também catalogado nesta auditoria) força FALHA_INESPERADA mesmo que seja só 1 em 99 -- nunca escondido na média.
- `compute_staleness_warning(...)`: comparação ao vivo (nunca gravada, recalculada a cada refresh) entre o último diagnóstico e o horário agendado -- acima de 30h sem execução, avisa que a atualização agendada pode não ter rodado (máquina desligada/hibernando -- RESIL-003, TASKS.md, limitação já conhecida e aceita do agendador do Windows, nunca tratado como bug novo).

**Onde o diagnóstico é gravado:** dentro de `run_once` (sucesso e falha, cobre login/censo já que `GSUSAdapter.get_census()` faz login e roda dentro do try de `run_once`) e num wrapper em `update_flow.py` (só os poucos caminhos que rodam ANTES de `run_once` -- credencial ausente, `client.goto()`). Uma flag (`exc._gsus_diagnostic_written`) marcada na própria exceção evita registro duplicado quando a mesma exceção passa pelos dois pontos.

**UI:** nova faixa na tela principal (`_build_diagnostic_banner`), logo abaixo da barra de controle -- cor neutra/âmbar quando a causa é conhecida do GSUS e não precisa de atenção extra, vermelha quando `needs_attention=True` ou quando a atualização parece não ter rodado. O texto sempre deixa explícito quando "não é defeito deste programa".
...
## DEC-114 — Visual novo levado ao produto final: rebuild 1.1.0 nesta máquina, modelo movido para a instalação, diagnóstico enganoso registrado

**Contexto (2026-09-04):** a sessão anterior (Codex) entregou DEC-112/113 no código-fonte, mas abriu a versão de desenvolvimento (`pythonw -m app.main` do repositório) em vez do programa instalado. Como o repositório não tem `runtime/` nem Firefox (só o Chromium dos testes em `playwright-browsers/`), as 7 tentativas de "Atualizar agora" falharam antes de tocar o GSUS, e o modelo de 4,92 GB foi baixado para `models/` do repositório. O usuário pediu para levar o visual novo à versão final e funcional.

**Decisões:**
1. **Build a partir da worktree** `claude/session-continuation-300b65` (trabalho do Codex replicado byte a byte), não do checkout principal: o checkout principal tem Chromium em `playwright-browsers/`, que iria parar dentro do instalador (DEC-073 proíbe), e continua com o trabalho não commitado do Codex intocado.
2. **Firefox e `runtime/` reaproveitados do app instalado** (`%LOCALAPPDATA%\Programs\GSUS Auditoria`), em vez de `playwright install firefox` e novo download do llama.cpp: sem rede, sem variação de versão (firefox-1465 e os mesmos DLLs ggml já validados em produção, DEC-095/096).
3. **Inno Setup 6.7.3 via winget, escopo de usuário** (sem admin), com autorização explícita do usuário -- mesma escolha de DEC-074, nova máquina.
4. **Versão 1.1.0** (minor): só apresentação e ativos visuais mudaram; schema, regras clínicas, persistência, automação e integração com o GSUS intactos.
5. **Modelo movido, não copiado, para dentro da instalação:** 5,7 GB livres não comportam cópia (4,92 GB) nem um segundo download com margem; a instância dev foi um engano e não precisa do modelo. O arquivo foi validado antes (cabeçalho GGUF + carga real no `llama-server.exe`, `/health` e `/completion` OK). Reversível com um `Move-Item` de volta.
6. **Tarefa agendada NÃO criada pelo agente:** é configuração persistente do Windows; a via oficial (Configurações → Concluir no build empacotado, `setup_window.py`) registra com o caminho certo do `.exe` e deixa rastro no log.
7. **`setup/` gitignored:** instaladores de 100+ MB (entregues ou recebidos) nunca devem entrar no repositório.

**Verificação:** PyInstaller 29 s / 448 MB / layout plano; teste de fumaça do `.exe` empacotado com pasta de dados temporária (vivo após 14 s, 0 stderr, dashboard + matplotlib + ícone conferidos em captura); Inno Setup 74 s / 120,0 MB / SHA-256 `E2D0E140807DE7F2D57DCAC5406848F95043DE14380E8B4E5B6C169DD52008C5`; instalação silenciosa exit 0, "GSUS Auditoria versão 1.1.0" no registro, 1711 arquivos, hashes do `.exe`, `python313.dll`, `base_library.zip`, `llama-server.exe` e `firefox.exe` idênticos ao dist; dados reais intactos. Suíte: 420 aprovados. O teste de fumaça do `.exe` **instalado** foi bloqueado pelo classificador de permissões do agente -- coberto pela equivalência de hash com o dist testado.
...
## DEC-116 — Encerrar a auditoria em andamento (cancelamento cooperativo) e interruptor navegador visível / segundo plano

**Pedido do usuário (2026-09-04):** botão para encerrar o processo de auditoria e um interruptor entre execução com o navegador visível e em segundo plano, com testes e novo instalador. Motivação real do mesmo dia: uma auditoria de 80 pacientes precisou ser parada matando 10 processos por fora.

**Decisões:**
1. **Cancelamento cooperativo, nunca abortivo.** Um `threading.Event` é checado só em pontos seguros: antes do censo, entre pacientes (Fase 1) e entre itens (Fase 2). Motivos: (a) a integridade já é por paciente (`mark_done`/`save_patient_state` commitam por paciente, DEC-095) -- parar ENTRE pacientes garante zero estado meio-gravado; (b) a API síncrona do Playwright não é thread-safe -- fechar o navegador de outra thread no meio de uma ação é justamente o tipo de travamento que o projeto já sofreu; (c) matar o processo continua funcionando como último recurso, mas deixa de ser o único caminho.
2. **Granularidade: por paciente na Fase 1, imediata na Fase 2.** O paciente em coleta termina (segundos a ~1 min, dependendo do GSUS) -- o status na tela avisa isso. Na Fase 2, `run_once` chama `llm.stop()` na hora: sem isso o botão só responderia depois da análise atual terminar (minutos em CPU); a thread trata a chamada derrubada como falha isolada do paciente (RF-12) e, vendo o evento setado, descarta o resto da fila -- nada é marcado analisado sem ter terminado, e o backlog (DEC-090) recoloca todos na próxima execução. Alternativa considerada e adiada: cancelar também entre DIAS dentro de `records.extract_notes` (resposta em segundos) -- exigiria tratar paciente meio-coletado (hash de dias já gravados x regras rodando sobre histórico parcial); registrado como melhoria possível, não bloqueador.
3. **Run `CANCELLED`, diagnóstico `CANCELADA` neutro, sem relatório parcial nem snapshot.** Regravar `relatorio.html` com 3 de 80 pacientes substituiria o relatório completo do dia por uma fatia; um snapshot parcial poluiria a tendência de dias vermelhos. `RunResult.status` deixa a UI distinguir "Atualizado" de "Interrompida pelo usuário" sem exceção (cancelar não é erro). `finish_run` já aceita qualquer status; nada no dashboard filtra por `COMPLETED`.
4. **Segundo plano = `headless=True`, opt-in, persistido, padrão visível.** DEC-077 provou que o GSUS bloqueia navegador automatizado oculto (login travava 100% numa máquina limpa) -- por isso o padrão continua visível, o modo oculto é escolha explícita do usuário, e a falha nesse modo mostra a dica de voltar. Alternativas rejeitadas: janela visível porém minimizada (as verificações de acionabilidade do Playwright dependem de `requestAnimationFrame`, que para em janela oculta -- risco alto de travar as ações, sem ganho sobre o headless); mover a janela para outra área de trabalho virtual (exótico, dependente de API não documentada). O modo em segundo plano NÃO foi validado contra o GSUS real nesta sessão -- decisão do usuário quando rodar.
5. **Interruptor desenhado em Canvas** (`app/ui/widgets.py::ToggleSwitch`, mesmo princípio dos ícones da DEC-113: sem fonte, arquivo externo ou pacote novo; o `ttk` tema `clam` não tem interruptor). Persistido no clique (`config.save_config`) para valer na tarefa agendada, que passa pelo mesmo `update_flow`. Bloqueado durante a atualização -- a escolha só vale na próxima.

**Verificação:** +11 testes (E2E: cancelamento no meio do 1º paciente prova `records.calls == 1`, run `CANCELLED`, 2 pacientes ainda `PENDING`, `llm.stopped`, 0 análises, relatório não gravado, 0 snapshots, diagnóstico `CANCELADA` sem atenção, thread da Fase 2 encerrada; cancelamento antes do censo nunca toca o censo nem cria run; regressão sem evento; config: chave ausente = True, round-trip False; cliente: `headless` chega no `firefox.launch` via Playwright fake; UI: botão apagado em repouso, ativo durante, clique único, status final "Interrompida pelo usuário — N/3", relatório ausente; interruptor persiste em `config.json` e dirige `headless=[True, False]` em duas atualizações; dica aparece na falha em segundo plano; diagnóstico). Suíte inteira aprovada; `.exe` empacotado testado com pasta de dados temporária; instalador 120,0 MB, SHA-256 `2CB17C12BC17FF95CDFDE9C2DA2ECB7B7A3C897B87BDFED6D77C6F60F7AE9718`. Achado de implementação: `tk.Canvas` reserva `self._w` como caminho Tcl do widget -- guardar a largura ali quebra qualquer chamada seguinte (`invalid command name "40"`).

**Impacto:** `app/config.py` (`browser_visible`), `app/gsus/client.py` (`headless`), `app/update_flow.py` (`cancel_event`, `headless`), `app/orchestrator.py` (`cancel_event`, `RunResult.status`, run `CANCELLED`), `app/analysis/run_diagnosis.py` (`OUTCOME_CANCELADA`, `describe_cancelled_run`), `app/storage/repository.py` (`RUN_STATUS_CANCELLED`), `app/ui/main_window.py` (botão, interruptor, status), `app/ui/widgets.py` (novo), `installer/gsus-auditoria.iss` (1.2.0), testes. Nenhuma mudança em schema, regras clínicas ou extração.

---

## DEC-117 — Disjuntor de GSUS sem responder à busca de prontuário: re-login após 3 pacientes seguidos, interrupção após 6, e erro com a causa certa

**Contexto (2026-09-04, log real):** 44 pacientes seguidos falharam no menu "Atendimento → Pesquisar Prontuário" (2,5 min cada, 110 min para 3 sucessos) com a sessão ainda "logada" -- ver CURRENT_STATE do dia. O DEC-103 já tinha classificado esse padrão como instabilidade do GSUS e o código já retentava corretamente (DEC-081/102), mas: (a) a mensagem por paciente dizia "nenhuma internação encontrada", escondendo a causa; (b) nenhuma alavanca entrava em ação quando a instabilidade durava horas -- o único detector de sessão morta (DEC-083/084) depende de marcadores de tela que não apareceram.

**Decisões:**
1. **Classe própria `GSUSSearchUnresponsiveError` (subclasse de `GSUSRecordError`) só quando NENHUMA tentativa passou do menu.** É o único caso em que se sabe que a busca nem foi disparada -- causa GSUS/sessão, não paciente. Falhas mistas (alguma tentativa passou do menu e o marcador não apareceu) continuam genéricas, porque podem ser paciente sem internação atual (hipótese 2 do DEC-103, ainda em aberto). Subclasse: quem já tratava `GSUSRecordError` continua funcionando.
2. **Disjuntor no orchestrator, não no adapter.** O orchestrator já é o dono do isolamento por paciente (RF-12) e do encerramento antecipado (UI-006); o adapter só ganhou `reset_session()`, chamado via duck typing (`getattr`) -- fontes sintéticas de teste não precisam implementar.
3. **Limiares 3 (re-login) e 6 (interrupção).** Com 5×30 s por paciente, 3 = ~7,5 min e 6 = ~15 min de espera no pior caso. Re-login primeiro porque é a única alavanca disponível antes de desistir (abrir sessão nova numa aba nova, DEC-083's `_relogin`), uma vez por sequência; se três pacientes seguintes continuam sem resposta, a instabilidade é do GSUS e insistir só queima tempo. Qualquer paciente que passe do menu zera o contador e libera um novo re-login futuro.
4. **Interrupção = run `FAILED` + resultado, não exceção.** A Fase 1 não terminou (FAILED é honesto no banco), mas o caminho de saída é o mesmo do Encerrar (UI-006): IA em andamento derrubada, fila da IA descartada, sem relatório parcial nem snapshot, diagnóstico `FALHA_GSUS` com atenção e texto que diz quantos foram salvos e que a próxima atualização retoma. `RunResult.status="ABORTED_GSUS"` deixa a tela distinguir de "Atualizado" e de "Interrompida pelo usuário".
5. **Nada muda no caminho feliz:** contador só sobe com a classe nova; o caso "marcador não apareceu" segue exatamente como antes (DEC-102/103).
