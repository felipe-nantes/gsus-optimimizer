...
## 2026-09-04 — Botão "Encerrar" e interruptor navegador visível/segundo plano; instalador 1.2.0 (DEC-116)

**Pedido do usuário:** (1) botão para encerrar a auditoria em andamento; (2) interruptor entre execução com o navegador visível (1º plano) e sem visualização (segundo plano); testar e atualizar o instalador.

**Contexto que motivou:** no mesmo dia, uma auditoria real de 80 pacientes precisou ser interrompida matando o processo por fora (não existia botão) -- 10 processos (app, llama-server, driver do Playwright, 7 do Firefox) encerrados à força.

**Feito:**
- **Encerrar:** um `threading.Event` (`cancel_event`) atravessa `MainWindow._on_update` → `update_flow.run_update` → `orchestrator.run_once` → thread da Fase 2. Cancelamento COOPERATIVO: checado antes do censo, entre um paciente e outro na Fase 1 e entre um item e outro na Fase 2. O paciente em coleta no instante do clique termina inteiro (nada fica meio-gravado); a análise por IA em andamento é derrubada na hora (`llm.stop()`) em vez de esperar minutos; a run é marcada `CANCELLED`; o relatório do dia NÃO é regravado com fatia parcial (o anterior continua valendo); nenhum snapshot diário é gravado; diagnóstico `CANCELADA` neutro (nunca pede atenção). `RunResult.status` distingue `COMPLETED`/`CANCELLED`. Na tela: botão "ENCERRAR" (estilo `Danger.TButton`) só habilitado durante a atualização; status "Encerrando... aguardando o paciente atual terminar (pode levar até um minuto)"; ao fim "Interrompida pelo usuário — N/M pacientes processados".
- **Navegador visível / segundo plano:** `AppConfig.browser_visible` (padrão True; configs antigas sem a chave carregam o padrão), interruptor `ToggleSwitch` (`app/ui/widgets.py`, desenhado em Canvas, sem dependência nova) na barra de status, persistido em `config.json` no clique (vale também para a tarefa agendada), bloqueado durante uma atualização. `GSUSClient(headless=not browser_visible)`. Quando uma atualização em segundo plano falha, a tela acrescenta a dica de voltar para "Navegador visível" (DEC-077).
- **Versão 1.2.0.** Testes novos: +3 E2E (cancelamento no meio do 1º paciente; cancelamento antes do censo; regressão sem evento), +2 config, +2 cliente GSUS (Playwright fake), +3 UI (botão Encerrar com adapter lento; interruptor persistindo e dirigindo `headless`; dica na falha em segundo plano), +1 diagnóstico. Suíte: 336 unitários não visuais + 60 integração/E2E + 3 ícone + UI por arquivo (com reexecução isolada das instabilidades Tcl/Tk já conhecidas) -- todos aprovados; `compileall` e `git diff --check` OK. Achado durante os testes: um `tk.Canvas` NÃO pode guardar largura em `self._w` (é o caminho Tcl do widget no Tkinter) -- corrigido em `widgets.py`.
- **Build:** PyInstaller 52 s; teste de fumaça do `.exe` empacotado com pasta de dados temporária: vivo após 12 s, 0 linhas de stderr, botão "ENCERRAR" e interruptor renderizados (captura via `PrintWindow`). Inno Setup: `installer/output/GSUSAuditoria-Setup.exe` 120,0 MB, SHA-256 `2CB17C12BC17FF95CDFDE9C2DA2ECB7B7A3C897B87BDFED6D77C6F60F7AE9718`. Instalação: instalado silenciosamente nesta máquina (exit 0), versão 1.2.0 no registro do Windows, .exe/python313.dll/base_library.zip/llama-server.exe idênticos ao dist testado por hash, 1712 arquivos = dist, modelo/Firefox/runtime preservados, dados reais intactos.

**Limite conhecido (DEC-077):** "segundo plano" = `headless=True`, que em 2026-08-26 fez o login do GSUS travar 100% numa máquina limpa (bloqueio de navegador automatizado oculto). NÃO foi validado contra o GSUS real nesta sessão (execução real é decisão do usuário). O padrão continua "navegador visível"; se o login falhar em segundo plano, a própria tela orienta voltar.

**Pendente:** validação real do modo segundo plano pelo usuário (uma atualização com o interruptor desligado); commit deste ciclo; push (bloqueado para o agente, comando entregue ao usuário).

---

## 2026-09-04 — Disjuntor de GSUS sem responder, com re-login, e mensagem de erro distinta; instalador 1.3.0 (DEC-117)

**Contexto (log real do dia, versão 1.2.0):** execução das 11:55 com 183 pacientes. Login e censo OK; pacientes 1 a 5 normais; a partir das 12:00 o menu "Atendimento → Pesquisar Prontuário" parou de responder: cada paciente esgotou 5 tentativas de 30 s (2,5 min) e caiu como `GSUSRecordError: Nenhuma internação em andamento encontrada...`. 44 falhas seguidas em ~2 h (112 avisos de menu às 12h, 111 às 13h) para 3 sucessos; 131 pacientes nunca iniciados. Nenhum marcador de sessão expirada/aba crashada (DEC-083/084) apareceu, então `_ensure_login` nunca refez o login. O programa foi fechado pela janela às ~13:56 (run ficou `RUNNING`; o resume reclassificou o órfão na execução seguinte). Segunda tentativa às 14:30: login OK, tela de busca do censo sem responder 5× → `GSUSCensusError`, diagnóstico `FALHA_GSUS` correto.

**Pedido do usuário:** implementar as duas melhorias apontadas na análise do log, commitar, e iniciar uma auditoria real acompanhando o processo.

**Feito:**
- **Mensagem distinta:** `records.open_current_admission` agora conta as tentativas que morreram JÁ no menu; se NENHUMA passou do menu, levanta `GSUSSearchUnresponsiveError` ("A tela de busca de prontuário não respondeu em 5 tentativas... GSUS lento, instável ou sessão degradada") em vez da mensagem genérica de "nenhuma internação encontrada", que passava a ideia de que o paciente não tinha internação. Se ao menos uma tentativa passou do menu, continua o `GSUSRecordError` de sempre. A nova classe entra em `KNOWN_GSUS_ERROR_PATTERNS` (causa conhecida do GSUS no diagnóstico por paciente).
- **Disjuntor com re-login (orchestrator):** contador de `GSUSSearchUnresponsiveError` seguidos na Fase 1. Ao chegar a `GSUS_UNRESPONSIVE_RELOGIN_AFTER = 3`, pede `record_source.reset_session()` uma vez por sequência (novo método do `GSUSAdapter`: refaz o login numa aba nova mesmo sem marcador de expiração). Ao chegar a `GSUS_UNRESPONSIVE_ABORT_AFTER = 6`, interrompe a Fase 1: run `FAILED`, sem relatório parcial nem snapshot (mesma política do Encerrar), IA em andamento derrubada, diagnóstico `FALHA_GSUS` com atenção ("O GSUS parou de responder à busca de prontuário por 6 paciente(s) seguido(s), mesmo depois de refazer o login... N de M processados e salvos... tente de novo mais tarde"), `RunResult.status="ABORTED_GSUS"` e status na tela "Interrompida: o GSUS parou de responder à busca de prontuário — N/M pacientes processados". Qualquer paciente que passe do menu (sucesso, sem internação, ou outro erro) zera a contagem. Pior caso hoje: ~15 min em vez de 110.
- **Versão 1.3.0.** Testes: +3 records (menu nunca responde → classe nova; menu responde mas marcador não aparece → genérico; falhas mistas → genérico), +2 adapter (`reset_session` refaz login mesmo sem marcador; no-op antes do 1º login), +3 E2E (re-login após 3 e interrupção após 6 com run `FAILED`, 3 `PENDING`, IA parada, sem relatório/snapshot, diagnóstico com atenção, erros reconhecidos como GSUS; recuperação após re-login completa a run; sequência zera com outro tipo de erro no meio), +2 diagnóstico, +1 UI (status do disjuntor). Suíte: 343 unitários não visuais + 63 integração/E2E + 3 ícone + UI por arquivo (com reexecução isolada das instabilidades Tk) -- todos aprovados; `compileall` e `git diff --check` OK. Ajuste de teste: o caso do botão Encerrar passou a esperar um `threading.Event` sinalizado pelo adapter falso no início da coleta, em vez de um texto de status que podia ser sobrescrito no mesmo ciclo de poll.
...
## 2026-09-07 (2) — Dois achados na primeira execução real do 1.4.0: card certo mas fechado pelo acordeão, e clique de menu ambíguo no censo (DEC-120)

**Execução 1 (21:04):** migração aplicada no banco real (123 datas convertidas, 38 pendências de regra removidas), login em segundo plano OK, censo com 171 pacientes -- e o novo critério de card FUNCIONOU contra a tela real ("Ignorando 14 dia(s)... critério: cabeçalho da internação atual"). Mas todo paciente falhou logo depois com "Dias capturados: 0 de N" + `GSUSRecordError: Nenhuma evolução encontrada`. Causa: o acordeão de episódios do GSUS é EXCLUSIVO -- `_expand_all` deixa aberto o ÚLTIMO episódio clicado (em geral um antigo) e fecha a internação atual; o critério antigo ("card aberto") ficava, por acaso, com o episódio errado mas visível; o novo escolhe o certo, cujos dias estavam ocultos. Execução interrompida por mim após 7 pacientes em erro (os 7 voltam pela fila da próxima execução).

**Correção:** `records._ensure_episode_expanded(page, card_id)` -- abre SÓ o card identificado (cabeçalho `<card_id>Item`) se fechado e confirma pela geometria do corpo, chamado logo após a seleção e de novo no laço quando um dia do card está oculto. +1 teste com fake de acordeão (dia do card atual oculto até o cabeçalho ser clicado; nunca clica no episódio antigo).

**Execução 2 (21:11), após reinstalar:** abortada ANTES do censo: `strict mode violation: get_by_text("Pesquisar Internação", exact=True) resolved to 2 elements` -- o item de menu apareceu duplicado na página e o clique estrito falhou 5 vezes em 1 segundo. Não é do código do dia; era fragilidade latente em `census._navigate_to_search_screen` e `records._click_menu_to_search_screen` (mesmo padrão). Correção: `get_by_text(...).locator("visible=true").first.click()` -- primeiro elemento VISÍVEL com o texto (Playwright 1.48 não tem `filter(visible=)`). Fakes de teste ganharam `.locator()`/`.first`.

**Execução 3 (21:18), após reinstalar com o reabre-card:** de novo "0 de N" -- e SEM o aviso de "não foi possível reabrir": `_ensure_episode_expanded` usava `_is_expanded` (filhos OU altura), e o corpo do episódio colapsado JÁ traz todos os cabeçalhos de dia no DOM (muitos filhos, altura ~0) -> "aberto" sem estar. Correção: `_episode_body_open` decide só pela ALTURA (mesmo critério `> 60 px` da listagem inicial de dias). Fake do teste passou a devolver `[40, 10]` colapsado, reproduzindo o caso real. Interrompida por mim após 14 pacientes.

**Execução 4 (após nova reinstalação, SHA-256 `7486B22E439BCF6D182A75F2A838585432F0D202742A71E2CF5121FA24C741B9`):** EM ANDAMENTO às 21:36 (fase 1): 170 pacientes no censo; 17 concluídos, 7 falhas, 145 na fila. Critério 'cabeçalho da internação atual' em 100% dos pacientes; 376 evoluções gravadas, TODAS de 2026 (0 de episódio antigo); 0 pendências novas com data anterior à admissão ou em DD/MM -- objetivo do DEC-119 confirmado no banco real. As 7 falhas são TODAS admissões de 07/09 (dia da execução) sem nenhuma evolução gravada: card atual com um único dia que segue oculto mesmo após reabrir -> 'Nenhuma evolução encontrada para extrair' (6) e 'marcador não apareceu' (1); pacientes admitidos de 01 a 06/09 concluem normalmente. Na versão anterior esses casos eram 'processados' com dias de episódios antigos (dado errado); agora falham de forma honesta e são retomados na execução seguinte. Aviso explícito ('segue oculto após reabrir') adicionado no código para o próximo build -- o instalado (7486B22E) ainda pula em silêncio. Follow-up RESIL-015: classificar 'admissão do dia sem evolução acessível' como categoria benigna em vez de erro.

---

## 2026-09-07 (3) — DEC-120 adendo: o dia de HOJE era pulado em todo paciente; diagnóstico estrutural e relistagem dos dias

**Execução 4 (21:27):** a primeira que andou -- 93 concluídos, 21 falhas, 0 evoluções de episódio antigo, 0 pendências com data antiga/DD-MM. Mas o aviso novo mostrou "Dia 1/N do card da internação atual segue oculto após reabrir" em TODO paciente, e o banco confirmou o pior: 0 evoluções datadas de 07/09 em 95+ pacientes, contra 82 evoluções do próprio dia capturadas pelo fluxo antigo em 04/09. A hipótese de AJAX (`_wait_visible`, 8 s) não resolveu.

**Diagnóstico estrutural (script ad-hoc `diag_day1.py` num prontuário real, só ids/geometria/CSS, nunca texto):** antes de reabrir, o dia mais recente é `historicoEvolucao0Item` (o "dia que a página abre sozinha", DEC-046) e o card tem altura 0; DEPOIS de reabrir, o GSUS RE-RENDERIZA esse dia como `historicoEvolucao<N>Item` (id novo), visível, corpo já carregado (154 filhos, 8.213 px). Os dias com data no id mantêm o id. A lista de dias montada ANTES da reabertura apontava pra um elemento inexistente -> `is_visible()` False -> dia de hoje pulado. Correção: `_list_day_rows` extraído e chamado de novo logo após reabrir o card. +1 teste (fake cuja listagem muda de id após o clique no cabeçalho). Suíte não visual: 361 aprovados.
...
**Fechamento da execução das 11:43 (1.4.0 final, run `5de8302a`, 11:43-12:53):** 163 de 179 processados, 5 sem internação atual, 11 falhas -- 3 admissões de 08/09 sem card (caso RESIL-015), 1 `TargetClosedError` (aba do Firefox fechada no paciente 172) e 7 `GSUSLoginError` em sequência ("pop-up do sistema não abriu" no relogin automático, mesma instabilidade de 04/09). 13.545 evoluções gravadas na execução, 1.391 datadas de hoje, 0 de episódio antigo. Fase 2 morta desde 12:00 (incidente DEC-123): só 6 análises de IA. Diagnóstico FALHA_INESPERADA por causa do `TargetClosedError` (padrão não catalogado -- candidato a entrar em `KNOWN_GSUS_ERROR_PATTERNS` como instabilidade do navegador/GSUS, a decidir).

**Instalação 1.5.0 (12:54):** setup 120,0 MB, SHA-256 `7981DFB6BB867EA55DD288DC3E3E4A69F982F09EE30E49498011807D54ECBF5A`; exe instalado idêntico ao dist (`92810828D8C6E628C6D122A30CFFD65B5E8C1B9178F27718DE30ED7D2086BB57`), versão 1.5.0 no registro, modelo/runtime/Firefox preservados. Ensaio prévio da migração numa cópia do banco real: colunas novas OK, 10 runs RUNNING -> INTERRUPTED, 116 pacientes a resgatar pra IA (31 nunca analisados + 85 com evolução após a última análise).

**Validação real do 1.5.0 (auditoria lançada 12:54, run `fd74a9fb`, headless):** run `fd74a9fb` (12:56-14:05): RESIL-016 confirmado (9 runs -> INTERRUPTED no início), RESIL-015 confirmado (2 admissões de 08/09 viraram AWAITING_NOTES -- os mesmos pacientes que falhavam de manhã), 25 evoluções de hoje aos 20 pacientes, 0 falhas até o paciente 167 (164/174 concluídos, 1 sem internação). Achados: (a) paciente com 80 dias na internação atual (2,8 MB) perdeu 26 dias após um clique falhar no dia 54 -- dias não gravados são reabertos sozinhos na próxima execução; (b) no fim do censo (~170 prontuários) o menu de busca parou de responder, o relogin do DEC-117 na MESMA sessão falhou de novo ("pop-up não abriu", 8 de 8 no dia) e o disjuntor fechou a execução com 6 seguidos (FAILED/ABORTED_GSUS), sem resgate de IA -> RESIL-019/DEC-127 (relogin em contexto novo), instalador 1.5.1; (c) IA: 41 análises antes do abort, 0 EDD inferida com a regex 1.5.0 -- a medição real mostrou que a forma dominante era `previsão de alta: dia dd/mm/aa` (adendo DEC-126). A sessão de desenvolvimento caiu às ~14:06 e zerou a ref da branch, o índice do worktree e o CURRENT_STATE.md do working tree (NTFS após queda abrupta); recuperados em 09/09 pelo reflog (SHA 3bc8c51) e pelo commit -- objetos íntegros, 21 commits à frente de main. Execução agendada de 00:01 (09/09) falhou no login (GSUS não respondeu, FALHA_GSUS). 1.5.1 instalado em 09/09 (setup SHA-256 `BBA920D17B1301156079DA8E856A39186F5C9C31BC0B760235CE0BBA0213F83F`, exe `F5CFE90B...D35E` idêntico ao dist).
